...
class LabelingWidget(LabelDialog):
    ...
    def __init__(  # noqa: C901
        ...
        if hasattr(self.canvas, "ai_review_confirm_requested"):
            self.canvas.ai_review_confirm_requested.connect(
                self.confirm_ai_review_shape
            )
    ...
    def documentation(self):
        url = (
            "https://github.com/Luoxr520/code"  # NOQA
        )
    ...
    def is_ai_review_shape(self, shape):
        """Return True when a shape is a temporary AI review candidate."""
        if shape is None:
            return False
        if hasattr(self.canvas, "is_ai_review_shape"):
            return self.canvas.is_ai_review_shape(shape)
        label = getattr(shape, "label", "") or ""
        return isinstance(label, str) and label.startswith("AI_REVIEW_")

    def get_real_label_from_ai_review_shape(self, shape):
        """Infer the final usable label from AI_REVIEW_* metadata."""
        if shape is None:
            return None
        if hasattr(self.canvas, "get_ai_review_real_label"):
            label = self.canvas.get_ai_review_real_label(shape)
            if label:
                return label
    ...
    def _ensure_confirmed_label_registered(self, label):
        """Ensure the confirmed class appears in label list/history/color maps."""
        if not label:
            return
        if not self.unique_label_list.find_items_by_label(label):
            item = self.unique_label_list.create_item_from_label(label)
            self.unique_label_list.addItem(item)
            rgb = self._get_rgb_by_label(label, skip_label_info=True)
            self.unique_label_list.set_item_label(
                item, label, rgb, LABEL_OPACITY
            )
        if label not in self.label_info:
            rgb = self._get_rgb_by_label(label, skip_label_info=True)
            self.label_info[label] = dict(
                delete=False,
                value=None,
                color=list(rgb),
        ...
        self.label_dialog.add_label_history(label, update_last_label=True)

    def _remove_unused_ai_review_label(self, old_label):
        """Remove an AI_REVIEW_* label entry if no remaining shape uses it."""
        if not isinstance(old_label, str) or not old_label.startswith("AI_REVIEW_"):
            return
        for shape in getattr(self.canvas, "shapes", []):
            if getattr(shape, "label", None) == old_label:
                return
        for item in list(self.unique_label_list.find_items_by_label(old_label)):
            row = self.unique_label_list.row(item)
            if row >= 0:
                self.unique_label_list.takeItem(row)
        self.label_info.pop(old_label, None)

    def _refresh_confirmed_ai_review_shape_item(self, shape):
        """Refresh object list row, shape color, filters and optional attributes UI."""
        self._update_shape_color(shape)
        item = self.label_list.find_item_by_shape(shape)
        if item is not None:
            if shape.group_id is None:
        ...
        self._refresh_shape_filters()
        try:
            selected_idx = self.canvas.shapes.index(shape)
            if self.attributes:
                self.update_attributes(selected_idx)
        except Exception:
            pass
        try:
            self.canvas.request_ai_review_confirm_button_sync()
        except Exception:
            pass
        self.canvas.update()

    def confirm_ai_review_shape(self, shape):
        """Convert an AI_REVIEW_* temporary box into a normal annotation."""
        if shape is None or not self.is_ai_review_shape(shape):
            return

        real_label = self.get_real_label_from_ai_review_shape(shape)
        if not real_label:
            self.status(self.tr("无法确认：未找到可用的正式标签"))
            return

        old_label = getattr(shape, "label", "") or ""
        self.canvas.store_shapes()

        shape.label = real_label
        if hasattr(shape, "score"):
            shape.score = None
        if hasattr(shape, "difficult"):
        ...
        if hasattr(shape, "chatbot_review"):
            try:
                shape.chatbot_review = {}
            except Exception:
                pass

        self._ensure_confirmed_label_registered(real_label)
        self._remove_unused_ai_review_label(old_label)
        self._refresh_confirmed_ai_review_shape_item(shape)
        self.set_dirty()
        try:
            self.status(
                self.tr("已确认 AI 临时框：%s → %s") % (old_label, real_label)
            )
        except Exception:
            pass
