...
class Canvas(
    ...
    ai_review_confirm_requested = QtCore.pyqtSignal(object)
    ...
    def __init__(self, *args, **kwargs):
        ...
        # AI_REVIEW_* temporary-box confirmation buttons.
        # They are QWidget children over the canvas, so they can be clicked
        # directly without changing the normal box editing/moving workflow.
        self.ai_review_confirm_buttons = {}
        self._ai_review_button_sync_pending = False
    ...
    # ------------------------------------------------------------------
    # AI_REVIEW_* temporary-box confirmation helpers
    # ------------------------------------------------------------------
    def is_ai_review_shape(self, shape):
        """Return True when a shape is an AI temporary review candidate."""
        if shape is None:
            return False
        label = getattr(shape, "label", "") or ""
        attributes = getattr(shape, "attributes", {}) or {}
        review_meta = getattr(shape, "chatbot_review", {}) or {}
        if not isinstance(attributes, dict):
            attributes = {}
        if not isinstance(review_meta, dict):
            review_meta = {}
        source = attributes.get("source") or review_meta.get("source")
        return (
            isinstance(label, str)
            and label.startswith("AI_REVIEW_")
        ) or source in {"ai_review", "chatbot_review", "AI_REVIEW"}

    def get_ai_review_real_label(self, shape):
        """Infer the final label from an AI_REVIEW_* temporary label."""
        if shape is None:
            return None
        attributes = getattr(shape, "attributes", {}) or {}
        review_meta = getattr(shape, "chatbot_review", {}) or {}
        ...
        for key in (
            ...
            value = attributes.get(key) or review_meta.get(key)
            if value:
        ...
        if not isinstance(label, str):
            return None
        if not label.startswith("AI_REVIEW_"):
            return label.strip() or None

        rest = label[len("AI_REVIEW_") :].strip()
        for action in (
            "false_positive",
            "wrong_class",
            "box_adjust",
            ...
            if rest.startswith(prefix):
                return rest[len(prefix) :].strip() or None
        return rest.strip() or None

    def _ai_review_shape_bounds(self, shape):
    ...
    def _ai_review_image_to_widget_point(self, x, y):
        """Map image coordinates to canvas widget coordinates."""
        try:
            offset = self.offset_to_center()
        except Exception:
            offset = QtCore.QPointF(0, 0)
        scale = getattr(self, "scale", 1.0) or 1.0
        # transform_pos() is: widget / scale - offset, so the inverse is
        # (image + offset) * scale. Keeping this inverse exact avoids button
        # drift when the image is centered or zoomed.
        return QtCore.QPoint(
            int(round((float(x) + offset.x()) * scale)),
            int(round((float(y) + offset.y()) * scale)),
        )

    def clear_ai_review_confirm_buttons(self):
        """Hide and delete all AI review confirmation buttons."""
        for _, button in list(getattr(self, "ai_review_confirm_buttons", {}).values()):
            try:
                button.hide()
                button.deleteLater()
            except Exception:
    ...
    def request_ai_review_confirm_button_sync(self):
        """Defer button layout until the current paint/update event finishes."""
        if getattr(self, "_ai_review_button_sync_pending", False):
            return
        self._ai_review_button_sync_pending = True
        QtCore.QTimer.singleShot(0, self.sync_ai_review_confirm_buttons)

    def sync_ai_review_confirm_buttons(self):
        """Create/update small confirm buttons over visible AI_REVIEW_* boxes."""
        self._ai_review_button_sync_pending = False
        if not self.isVisible():
            return
        if getattr(self, "is_loading", False):
            self.clear_ai_review_confirm_buttons()
            return
        ...
        active_keys = set()
        buttons = getattr(self, "ai_review_confirm_buttons", {})
        if not isinstance(buttons, dict):
            buttons = {}
            self.ai_review_confirm_buttons = buttons

        for shape in getattr(self, "shapes", []):
            try:
                if not self.is_visible(shape) or not self.is_ai_review_shape(shape):
                    continue
                bounds = self._ai_review_shape_bounds(shape)
                if not bounds:
                    continue
                x1, y1, x2, y2 = bounds
                top_left = self._ai_review_image_to_widget_point(x1, y1)
                top_right = self._ai_review_image_to_widget_point(x2, y1)
                bottom_right = self._ai_review_image_to_widget_point(x2, y2)

                button_w, button_h = 56, 24
                x = top_right.x() - button_w
                y = top_right.y() - button_h - 4
                if y < 0:
                    y = top_right.y() + 4
                if y + button_h > self.height():
                    y = max(0, bottom_right.y() - button_h - 4)
                if x < 0:
                    x = top_left.x()
                x = max(0, min(max(0, self.width() - button_w), x))

                key = id(shape)
                active_keys.add(key)
                if key in buttons:
                    _, button = buttons[key]
                else:
                    button = QtWidgets.QPushButton(self.tr("确认"), self)
                    button.setObjectName("aiReviewConfirmButton")
                    button.setCursor(QtCore.Qt.CursorShape.PointingHandCursor)
                    button.setToolTip(self.tr("确认该 AI 临时框并转为正式标注"))
                    button.setFixedSize(button_w, button_h)
                    button.setStyleSheet(
                        "QPushButton#aiReviewConfirmButton {"
                        "background-color: rgba(34, 160, 107, 232);"
                        "color: white; border: 1px solid rgba(255,255,255,210);"
                        "border-radius: 5px; font-weight: 600; font-size: 12px;"
                        "padding: 0px; }"
                        "QPushButton#aiReviewConfirmButton:hover {"
                        "background-color: rgba(21, 128, 80, 248); }"
                    )
                    button.clicked.connect(
                        lambda checked=False, s=shape: self.ai_review_confirm_requested.emit(s)
                    )
                    buttons[key] = (shape, button)
                button.move(x, y)
                button.show()
                button.raise_()
            except Exception:
    ...
    def resizeEvent(self, event):
        """Keep AI_REVIEW_* confirm buttons aligned when the canvas is resized."""
        try:
            super().resizeEvent(event)
        except AttributeError:
            pass
        self.request_ai_review_confirm_button_sync()
