...
from PyQt6.QtCore import QTimer, Qt, QSize, QPoint, QEvent, QPropertyAnimation
...
class ChatbotDialog(QDialog):
    ...
    def get_annotation_file_path(self, image_path):
        """Return the same-name SelfEvolvingRecognition JSON file path for an image."""
        if not image_path:
            return None
    ...
    def points_to_bbox_text(self, points):
        """Convert label points to a compact bbox-like text description."""
        if not points:
            return None
        ...
        try:
            ...
            return (
                f"x1={min(xs):.1f}, y1={min(ys):.1f}, "
                f"x2={max(xs):.1f}, y2={max(ys):.1f}"
            )
        except Exception:
            return str(points)

    def load_annotation_context(self, image_path, max_shapes=80):
        """
        Read the current image's same-name SelfEvolvingRecognition JSON file and convert
        its annotation data into a concise text context for the vision LLM.
        """
        json_path = self.get_annotation_file_path(image_path)
        if not json_path:
            return None
        ...
        try:
            ...
            logger.warning(f"Failed to read annotation JSON {json_path}: {e}")
            return f"当前图片存在同名标注文件，但读取失败：{e}"

        if not isinstance(data, dict):
            return "当前图片存在同名标注文件，但文件内容不是有效的 JSON 对象。"
        ...
        image_path_in_json = data.get("imagePath", None)

        lines = []
        lines.append("以下是当前图片对应的 SelfEvolvingRecognition/YOLO 标注结果。")
        lines.append("注意：这些标注可能来自模型自动检测或人工复核，不保证完全正确，只能作为待核对参考。")
        lines.append(f"标注文件：{os.path.basename(json_path)}")

        if image_path_in_json:
            lines.append(f"标注对应图片：{image_path_in_json}")
        if image_width and image_height:
            lines.append(f"图像尺寸：{image_width} x {image_height}")
        if checked is not None:
            lines.append(f"人工复核状态：{'已复核' if checked else '未复核'}")
        if description:
            lines.append(f"图像整体描述：{description}")

        if not shapes:
            lines.append("标注对象数量：0。当前标注文件中没有 shapes 对象。")
            return "\n".join(lines)

        lines.append(f"标注对象数量：{len(shapes)}")
        lines.append("标注对象列表：")

        shown_shapes = shapes[:max_shapes]
        for index, shape in enumerate(shown_shapes, start=1):
            if not isinstance(shape, dict):
                lines.append(f"{index}. 非标准标注对象：{shape}")
                continue

            label = shape.get("label", "未知类别")
            shape_type = shape.get("shape_type", "unknown")
            points = shape.get("points", []) or []
            score = shape.get("score", None)
            difficult = shape.get("difficult", False)
            group_id = shape.get("group_id", None)
            shape_desc = shape.get("description", "")
            flags = shape.get("flags", {}) or {}
            attributes = shape.get("attributes", {}) or {}
            bbox_text = self.points_to_bbox_text(points)

            item = f"{index}. 类别：{label}；标注类型：{shape_type}"
            if bbox_text:
                item += f"；位置：{bbox_text}"
            ...
            if group_id is not None:
                item += f"；组ID：{group_id}"
            if shape_desc:
                item += f"；对象描述：{shape_desc}"
            if flags:
                item += f"；flags：{json.dumps(flags, ensure_ascii=False)}"
            if attributes:
                item += f"；attributes：{json.dumps(attributes, ensure_ascii=False)}"

            lines.append(item)

        if len(shapes) > max_shapes:
            lines.append(
                f"由于标注对象较多，仅列出前 {max_shapes} 个，剩余 {len(shapes) - max_shapes} 个未展开。"
            )

        return "\n".join(lines)

    def build_message_text_with_annotation_context(self, message_text, image_path):
        """
        Append image-annotation review instructions to the model input.
        The visible chat history remains unchanged.
        """
        if not image_path:
            return message_text

        if "@image" not in message_text and "<image>" not in message_text:
            return message_text

        annotation_context = self.load_annotation_context(image_path)
        if not annotation_context:
            annotation_context = (
                "当前图片没有找到同名 SelfEvolvingRecognition JSON 标注文件。"
                "请仅根据图片独立判断；只有在目标清晰、位置能够较准确定位时，才输出待复核标注候选。"
            )

        return (
            f"{message_text}\n\n"
            "【当前 YOLO/SelfEvolvingRecognition 标注上下文】\n"
            f"{annotation_context}\n\n"
            "【回答语言与结构要求】\n"
            "1. 必须使用中文回答。\n"
            "2. 不要盲信标注文件；标注文件可能来自 YOLO 自动检测，可能存在漏检、误检、类别错误、重复框、框偏移或框大小不准。\n"
            "3. 先根据图片本身进行独立视觉判断，再对照当前标注文件。\n"
            "4. 回答尽量按以下结构组织：图片独立判断、当前标注结果、差异分析、人工微调建议。\n"
            "5. 如果图片视觉判断和标注文件不一致，需要明确说明是哪一类问题：漏检、误检、类别错误、重复框、框偏移、框过大或框过小。\n"
            "6. 所有待标注区域都只是临时复核候选，不是最终确认标签，最终仍由人工确认、修改或删除。\n\n"
            "【画布临时标注输出规则】\n"
            "在回答最后，必须额外输出一个 JSON 代码块供程序读取；聊天窗口会自动隐藏该 JSON。\n"
            "如果没有需要自动画到画布上的候选框，请输出空列表。\n"
            "bbox 必须是原始图片像素坐标，格式为 [x1, y1, x2, y2]，不是百分比坐标。\n"
            "坐标系规则：左上角是 (0,0)，x 向右增大，y 向下增大。\n"
            "只输出你能较准确定位的候选框；如果只能判断存在差异但无法准确定位，请在中文回答中说明，不要为了凑 JSON 而猜框。\n"
            "不要给已经正确的 YOLO 框重复输出候选框。\n"
            "对很小、被遮挡、模糊或位置不确定的目标，除非你能给出覆盖目标主体的外接框，否则不要输出候选框。\n"
            "confidence 表示你对该候选框位置和类别都较可靠的程度；低于 0.85 的候选框程序会自动忽略。\n"
            "使用这个固定 schema：\n"
            "```json\n"
            "{\n"
            "  \"ser_review_candidates\": [\n"
            "    {\n"
            "      \"action\": \"missing|false_positive|wrong_class|box_adjust|duplicate\",\n"
            "      \"label\": \"目标类别名称，例如 person\",\n"
            "      \"bbox\": [x1, y1, x2, y2],\n"
            "      \"confidence\": 0.0,\n"
            "      \"reason\": \"简短中文原因\"\n"
            "    }\n"
    ...
    def strip_review_candidates_json(self, text):
        """Hide the machine-readable review-candidate JSON from chat display."""
        if not text or "ser_review_candidates" not in text:
            return text

        def remove_if_review_block(match):
            block = match.group(0)
            return "" if "ser_review_candidates" in block else block

        cleaned = re.sub(
            r"```(?:json)?\s*[\s\S]*?```",
            remove_if_review_block,
            text,
            flags=re.IGNORECASE,
        ).strip()
        cleaned = re.sub(r"\n{3,}", "\n\n", cleaned)
    ...
    def extract_json_object_near_key(self, text, key):
        """Extract a balanced JSON object that contains key."""
    ...
    def extract_review_candidates_from_response(self, text):
        """Parse review candidates from the assistant response."""
        if not text or "ser_review_candidates" not in text:
            return []
        ...
        for match in re.finditer(
            ...
            if "ser_review_candidates" in block:
                json_candidates.append(block)

        loose_json = self.extract_json_object_near_key(
            text, "ser_review_candidates"
        )
        if loose_json:
            json_candidates.append(loose_json)
        ...
        for json_text in json_candidates:
            ...
            candidates = data.get("ser_review_candidates", [])
            if isinstance(candidates, list):
                return candidates

        return []

    def get_image_size_for_review(self, image_path, data=None):
        """Get image size from JSON data first, then from the image file."""
    ...
    def normalize_review_bbox(self, bbox, image_width=None, image_height=None):
        """Normalize bbox into clipped [x1, y1, x2, y2] absolute coordinates."""
    ...
    def is_chatbot_review_shape(self, shape):
        """Return True for temporary review shapes previously added by chatbot."""
        if not isinstance(shape, dict):
            return False
        label = str(shape.get("label", ""))
        attributes = shape.get("attributes", {}) or {}
        review_meta = shape.get("chatbot_review", {}) or {}
        return (
            label.startswith("AI_REVIEW_")
            or attributes.get("source") == "chatbot_review"
            or review_meta.get("source") == "chatbot_review"
        )

    def candidate_to_review_shape(self, candidate, index, image_width, image_height):
        """Convert one parsed model candidate into an SelfEvolvingRecognition rectangle shape."""
        if not isinstance(candidate, dict):
            return None

        action = str(candidate.get("action", "review") or "review").strip()
        action = re.sub(r"[^A-Za-z0-9_\-]+", "_", action)[:40] or "review"
        label = str(candidate.get("label", "object") or "object").strip()
        label = re.sub(r"\s+", "_", label)[:80] or "object"
        bbox = candidate.get("bbox") or candidate.get("box")
        if bbox is None and isinstance(candidate.get("points"), list):
        ...
        bbox = self.normalize_review_bbox(bbox, image_width, image_height)
        if not bbox:
            return None
        ...
        # Vision-language models are often weak at exact pixel localization.
        # To avoid polluting the labeling canvas with obviously wrong boxes,
        # only high-confidence candidates are written back as temporary marks.
        if confidence is None or confidence < 0.50:
            return None

        review_label = f"AI_REVIEW_{action}_{label}"
        return {
            "label": review_label,
            "score": confidence,
            "points": [
            ...
            "group_id": None,
            # Keep the canvas clean: detailed reasons are stored in the JSON
            # metadata below, not rendered as a large overlay on the image.
            "description": "",
            "difficult": True,
    ...
    def load_or_create_label_data_for_review(self, image_path):
        """Load same-name label JSON, or create a minimal one for review marks."""
        json_path = self.get_annotation_file_path(image_path)
        data = None
        ...
        if json_path:
            try:
                ...
                logger.warning(f"Failed to load annotation JSON for review: {e}")
                data = None

        if not isinstance(data, dict):
            base, _ = os.path.splitext(image_path)
            json_path = base + ".json"
            image_width, image_height = self.get_image_size_for_review(image_path)
            data = {
                "version": __version__,
                "flags": {},
                "shapes": [],
                ...
                "imageData": None,
                "imageHeight": image_height or 0,
                "imageWidth": image_width or 0,
                "description": "",
                "checked": False,
            }
        ...
        if "flags" not in data or not isinstance(data.get("flags"), dict):
            data["flags"] = {}
        ...
        image_width, image_height = self.get_image_size_for_review(image_path, data)
        if image_width and image_height:
            data["imageWidth"] = image_width
            data["imageHeight"] = image_height

        return json_path, data, image_width, image_height

    def apply_review_candidates_to_current_image(self, assistant_text):
        """
        Add temporary AI_REVIEW_* rectangles to the current image label JSON.
        Existing temporary chatbot review marks are cleared first, so a new
        answer will not leave outdated or wrongly placed AI_REVIEW boxes behind.
        """
        image_path = self.parent().filename if self.parent() else None
        if not image_path or not os.path.exists(image_path):
            return 0

        if not assistant_text or "ser_review_candidates" not in assistant_text:
            return 0

        candidates = self.extract_review_candidates_from_response(assistant_text)

        json_path, data, image_width, image_height = self.load_or_create_label_data_for_review(
            image_path
        )
        if not json_path:
            return 0

        existing_shapes = data.get("shapes", []) or []
        kept_shapes = [
            shape for shape in existing_shapes
            if not self.is_chatbot_review_shape(shape)
        ]
        removed = len(existing_shapes) - len(kept_shapes)
        data["shapes"] = kept_shapes

        added = 0
        for index, candidate in enumerate(candidates, start=1):
            shape = self.candidate_to_review_shape(
                candidate, index, image_width, image_height
            )
            if shape:
        ...
        review_note = (
            f"Chatbot refreshed temporary AI_REVIEW candidates. "
            f"Added: {added}; removed old: {removed}. "
            "They are not confirmed labels and require manual verification."
        )
        data["chatbot_review_note"] = review_note
        data["chatbot_review_candidates"] = candidates

        try:
            with open(json_path, "w", encoding="utf-8") as f:
                json.dump(data, f, ensure_ascii=False, indent=2)
        except Exception as e:
            logger.error(f"Failed to save chatbot review candidates: {e}")
            return 0

        return added + removed

    def return_to_labeling_canvas_after_review(self, image_path):
        """Reload current image so newly added review marks appear on the canvas."""
        try:
            if not self.parent() or not image_path:
                return
            self.parent().load_file(image_path)
            QApplication.processEvents()
            self.update_image_preview()
            # Keep the chatbot dialog open. The user can read the Chinese
            # review explanation while checking the temporary canvas marks.
            self.raise_()
            self.activateWindow()
        except Exception as e:
            logger.error(f"Failed to reload labeling canvas: {e}")
    ...
    def on_stream_finished(self, success):
        ...
        if success and self.loading_message:
            ...
            display_text = self.strip_review_candidates_json(raw_final_text)

            # Store reference to loading message for removal
            loading_message_to_remove = self.loading_message
            self.loading_message = None

            # Add the final message first. The machine-readable JSON block is
            # hidden from the chat UI, but it is still used below to create
            # temporary review marks on the labeling canvas.
            self.add_message("assistant", display_text)

            review_added_count = self.apply_review_candidates_to_current_image(
                raw_final_text
            )
            ...
            if review_added_count > 0 and self.parent().filename:
                self.return_to_labeling_canvas_after_review(
                    self.parent().filename
                )
            elif self.parent().filename:
                # Keep original behavior when no canvas review mark was saved.
    ...
    def process_single_image(self, filename):
        ...
        try:
            ...
            if "<image>" in self.batch_prompt:
                ...
                prompt_text = self.build_message_text_with_annotation_context(
                    self.batch_prompt, filename
                )
                messages.append(
                    {
                        "role": "user",
                ...
                if msg["image"]:
                    try:
                        with open(msg["image"], "rb") as image_file:
                            ...
                            message_text = self.build_message_text_with_annotation_context(
